...
3. Set the hostname of Identity Server:
...
6. Configure the following endpoints for the `private_key_jwt_authenticator` event listener:
    ...
    - Configure `TokenEndpointAlias` with the hostname of Identity Server.
    - Configure `notification_endpoint` with the hostname of API Manager.  
...
7. Configure the event publisher URL for adaptive authentication with the hostname of Identity Server.
