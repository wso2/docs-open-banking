...
Represents the bank accounts belonging to the user. `accounts` include an optional `selected` property, set to either `true` or `false`. If `true`, the account will be pre-selected when displayed, before any user action.
